...
def mapperF(data):
    ...
    if (data[5] >= thresholdZ):
        if(not R):
            print("Left Active")
            z = genMapper([-0.050, 0.100], [zValU, zValL], data[5])
            x = genMapper([-0.080, 0.080], [xValU, xValL], data[3])
    ...
    if (data[2] >= thresholdZ):
        if(not L):
            print("Right Active")
            z = genMapper([-0.050, 0.100], [zValU, zValL], data[2])
            x = genMapper([-0.080, 0.080], [xValU, xValL], data[0])
        ...
        R = False
    if (not L):
        updatePos([4, 10, 15], [inverse(zValU), inverse(zValU), zValU])
    elif (not R):
        updatePos([3,  9, 16], [zValU, zValU, inverse(zValU)])
...
def mapperT1(data):
    ...
    if(data[1] >= thresholdY and T1Flag):
        ...
        updatePos([4, 10, 15], [inverse(yValU), inverse(yValU), yValU])
    else:
        if(T1Change):
            T1Flag = not(T1Flag)
            T1Change = False
...
def initializer():
    ...
    updatePos([3, 15, 9],[Z]*3)
    updatePos([4, 16, 10], [inverse(Z)]*3)
    updatePos([11, 17, 5], [fixed]*3)
    updatePos([12, 18, 6], [inverse(fixed)]*3)
