...
if dxl_comm_result != COMM_SUCCESS:
    ...
    print("HELP Dynamixel#%d has been successfully connected" % DXL1_ID)
...
while 1:
    ...
    if testFlag:
        dxl_goal_position[index] = dxl_goal_position[index]+10
    else:
        dxl_goal_position[index] = dxl_goal_position[index]-10

    print("position: ", dxl_goal_position[index])
    
    # Allocate goal position value into byte array
    param_goal_position = [DXL_LOBYTE(DXL_LOWORD(dxl_goal_position[index])), DXL_HIBYTE(DXL_LOWORD(dxl_goal_position[index]))] #Keep a list of 2 (VIMP) 
    ...
    while 1:
        ...
        if dxl_comm_result != COMM_SUCCESS:
            print("%sPIDA" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
    ...
    if (dxl1_present_position > DXL_MAXIMUM_POSITION_VALUE ):
        print("greater 500")
        testFlag = False
    elif (dxl1_present_position < DXL_MINIMUM_POSITION_VALUE ):
        print("lesser 200")
        testFlag = True
